...
Saraga F, Wu CP, Zhang L, Skinner FK. Active Dendrites and Spike Propagation in Multi-compartment Models of Oriens-Lacunosum/Moleculare Hippocampal Interneurons.
...
### Notes from the ModelDB administrator

- **20150416**
  A bug noted by Mohamed Sherif and fix provided by Ted Carnevale updates the `IA.mod` file so that the a,b states change.

- **20160830**
  A bug noted by Andras Ecker and communicated by Marianne Bezaire was that `IAprox.mod` neglected to call the rates function. The previous fix (20150416) required slight changes (see below) in the IClamp current (adjustable in the Point Process Manager window which becomes available when you run the simulation with `mosinit.hoc`) for some of the generated figures to be similar to those in the paper:
  1) For 20150416 version in the 8B Long case the current needed to be increased from 0.2 to 0.27; however with this subsequent version (20160830) this is no longer necessary.
